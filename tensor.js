...
//for (i=0;i<6;i++)
//{
  testFolder="./data/training/"+arr[0];
  final=[];
  fs.readdir(testFolder, (err, files) => {
    files.forEach(file => {
      f=testFolder+"/"+file
      abc(f,file).then((data)=>
      {
  ...
  img.onload=()=>{
    ...
    singlePoseOutput(input).then((data)=>
    {
      ...
      var str=data.score+","+data.keypoints[0].position.x+","+data.keypoints[0].position.y+","+data.keypoints[0].score+","+
              ...
              data.keypoints[16].position.x+","+data.keypoints[16].position.y+","+data.keypoints[16].score+","+f1+",straight\n";
              console.log(str);
